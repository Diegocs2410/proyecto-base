...
function LoginForm() {
  ...
  const onSubmit = async (data: FormData) => {
    ...
    const resultado = await iniciarSesion(data.email, data.password, next);
  ...
  return (
    <div className="rounded-3xl border border-border bg-white p-8 shadow-sm">
      ...
      <form className="grid gap-4" onSubmit={handleSubmit(onSubmit)}>
        ...
        <div className="grid gap-1.5">
          <div className="flex items-center justify-between">
            ...
            <Link className="text-xs text-slate-500 underline-offset-4 hover:underline" href="/auth/recuperar">
      ...
      <p className="mt-6 text-center text-sm text-slate-500">
        ...
        <Link className="font-medium text-slate-900 underline-offset-4 hover:underline" href="/auth/registro">
          Regístrate gratis
        </Link>
      </p>
    </div>
